  ...
  M_coefficients(datafile("time_discretization/scheme", "ROS2"))
{
    ...
    // M_massMatrixNoBCs = assembler->assembleMassMatrix();
    //
...
solveTimestep(const double &time, double &dt)
{
    ...
    // M_assembler->setTimeAndPrevSolution(time, M_solution);
    //
    // MapEpetraPtr globalMap = M_assembler->getGlobalMap();
    // BlockMatrix globalMass = M_assembler->getMassMatrix();
    //
    // double diagonalCoefficient = 0.0;
    // BlockMatrix globalJac = M_assembler->getJacobianF(true, &diagonalCoefficient);
    ...
    //
    // VectorPtr Fder = M_assembler->computeFder_();
    //
    // for (int i = 0; i < s; i++)
    // {
    ...
    //     }
    //
    //     M_assembler->setTimeAndPrevSolution(time + dt * alphai, yTilde);
    //     VectorPtr F = M_assembler->computeF_();
    ...
    //     VectorEpetra prod = M_assembler->getGlobalMass() * (*sumStages);
    //     *F += prod;
    //
    ...
    //     M_assembler->applyBCsRhsRosenbrock(F, yTilde, time, dt,
    //                                              alphai, gammai);
