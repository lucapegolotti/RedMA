...
BlockMatrix
RBBases::
leftProject(BlockMatrix matrix, unsigned int ID)
{
...
RBMATRIX
RBBases::
leftProject(FEMATRIX matrix, unsigned int basisIndex, unsigned int ID)
{
    // DenseMatrix retMat;
    //
    // if (matrix.data())
...
BlockVector
RBBases::
projectOnLagrangeSpace(BlockVector vector)
{
    // if (vector.nRows() > 1)
...
BlockVector
RBBases::
leftProject(BlockVector vector, unsigned int ID)
{
...
DenseVector
RBBases::
leftProject(DistributedVector vector, unsigned int basisIndex, unsigned int ID)
{
    // if (vector.data())
    // {
    //     VectorEp resVector;
...
BlockMatrix
RBBases::
rightProject(BlockMatrix matrix, unsigned int ID)
{
    // BlockMatrix<DenseMatrix> projectedMatrix(matrix.nRows(), matrix.nCols());
    //
    // for (unsigned int i = 0; i < matrix.nRows(); i++)
...
DenseMatrix
RBBases::
rightProject(FEMATRIX matrix, unsigned int basisIndex, unsigned int ID)
{
    // DenseMatrix retMat;
    //
    // if (matrix.data())
...
DenseMatrix
RBBases::
matrixProject(FEMATRIX matrix, unsigned int basisIndexRow,
...
SHP(VECTOREPETRA)
RBBases::
reconstructFEFunction(RBVECTOR rbSolution, unsigned int index,
                      unsigned int ID)
{
