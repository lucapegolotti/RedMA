//
// Created by betti on 4/6/22.
//

#include "SnapshotsSteadySampler.h"
...
namespace RedMA
{
    ...
    takeSnapshots(const unsigned int &Nstart) {
        ...
        std::vector <std::map<std::string, double>> SnapshotsSamples =
                M_sampler.getSamples(M_numSamples, M_sampler.M_generatingVector,
                                     M_sampler.M_paramsNames, M_sampler.M_paramsBounds);
        ...
        for (unsigned int i = 0; i < M_numSamples; i++) {
            ...
            {
                array_params.push_back(it->second);
            }
            ...
            std::function < double(double) > outletDirichlet = [vec_inflow, numOutlet, inletBC] (double t)
                        { return (vec_inflow[0] + vec_inflow[1]) * inletBC(t); };
