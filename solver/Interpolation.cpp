...
void
Interpolation::
setup( const GetPot& datafile, parameterList_Type belosList )
{
    M_datafile = datafile;
...
double
Interpolation::
rbf (double x1, double y1, double z1, double x2, double y2, double z2, double radius)
{
...
void
Interpolation::
setVectors (const vectorPtr_Type& KnownField, const vectorPtr_Type& UnknownField )
{
...
buildOperators()
{
    M_comm = M_knownField->mapPtr()->commPtr();
...
void
Interpolation::
gatherVectorsAcrossProcessors( int* globalMarker, double* globalXs,
                                              double* globalYs, double* globalZs,
                                              std::vector<std::vector<int > >& conn )
{
    Int numtasks = M_knownField->mapPtr()->commPtr()->NumProc();
    M_pid = M_knownField->mapPtr()->commPtr()->MyPID();

    Int nNodes = M_xcoord_known.size();

    M_comm->MaxAll(&M_marker_known[0], globalMarker, nNodes);

    // this works as long as nodes are owned only by one processor (because value
    // are zero everywhere except on that processor)
    M_comm->SumAll(&M_xcoord_known[0], globalXs, nNodes);
    M_comm->SumAll(&M_ycoord_known[0], globalYs, nNodes);
    M_comm->SumAll(&M_zcoord_known[0], globalZs, nNodes);

    // this part is actually serial but this is the best we can do at the moment
    Int sizesConnectivity = M_dof_connectivity_known.size();
    ...
    for (Int i = 0; i < numtasks; i++)
    {
        ...
        for (Int j = 0; j < *sizeCurrConnectivity; j++)
        {
            ...
            if (M_pid == i)
                *sizeCurList = M_dof_connectivity_known[j].size();
            M_comm->Broadcast(sizeCurList, 1, i);

            Int* curList = new Int[*sizeCurList];
            // copy current list in the raw array
            if (i == M_pid)
            {
                for (Int k = 0; k < *sizeCurList; k++)
                {
                    curList[k] = M_dof_connectivity_known[j][k];
                }
            }
...
projectionOperator()
{
    ...
    Int* globalMarker = new Int[nNodes];
    Real* globalXs = new Real[nNodes];
    Real* globalYs = new Real[nNodes];
    Real* globalZs = new Real[nNodes];
    std::vector<std::vector<int > > dof_connectivity_known;
    gatherVectorsAcrossProcessors(globalMarker, globalXs, globalYs, globalZs,
                                  dof_connectivity_known);
    for (std::set<ID>::iterator it = M_GIdsUnknownMesh.begin(); it != M_GIdsUnknownMesh.end(); ++it)
    {
        ...
        {
            if ( globalMarker[j] == M_flag )
            {
                d = std::sqrt ( ( globalXs[j] - M_xcoord_unknown[*it] ) * ( globalXs[j] - M_xcoord_unknown[*it] ) +
                                ( globalYs[j] - M_ycoord_unknown[*it] ) * ( globalYs[j] - M_ycoord_unknown[*it] ) +
                                ( globalZs[j] - M_zcoord_unknown[*it] ) * ( globalZs[j] - M_zcoord_unknown[*it] ) );
                if (d < d_min)
                {
                    d_min = d;
                    nearestPoint = j;
    ...
    for ( int i = 0 ; i < LocalNodesNumber; ++i )
    {
        for ( int it = 0; it < MatrixGraph[i].size(); ++it )
        {
            Values  = rbf ( M_xcoord_unknown[GlobalID[i]],
                            ...
                            globalXs[MatrixGraph[i][it]],
                            globalYs[MatrixGraph[i][it]],
                            globalZs[MatrixGraph[i][it]],
...
double
Interpolation::
computeRBFradius_known ( const ID& index, std::vector<int> indexes )
{
    double r = 0;
    double r_max = 0;
...
double
Interpolation::
computeRBFradius_unknown ( const ID& index, std::vector<int> indexes )
{
    double r = 0;
    double r_max = 0;
    for ( int i = 0; i < indexes.size(); ++i )
...
interpolationOperator()
{
    ...
    int* globalMarker = new int[nNodes];
    double* globalXs = new double[nNodes];
    double* globalYs = new double[nNodes];
    double* globalZs = new double[nNodes];
    std::vector<std::vector<int > > dof_connectivity_known;
    gatherVectorsAcrossProcessors(globalMarker, globalXs, globalYs, globalZs,
                                  dof_connectivity_known);
...
void
Interpolation::
buildTableDofs_known ( const FESpacePtr_Type& fespace )
{
    ...
    for ( int i = 0; i < numBFaces; ++i )
    {
        if ( fespace->mesh()->boundaryFacet ( i ).markerID() == M_flag )
        {
            ...
            for (ID lDof1 = 0; lDof1 < localToGlobalMapOnBFacet1.size(); lDof1++)
            {
                ...
                // we do this because, even if the individual dof has a marker
                // different from that of the face, it does belong to the face
                // and we would probably like to interpolate over it (e.g.
                // when in fsi you have boundary conditions on the rings, the
...
void
Interpolation::
buildTableDofs_unknown ( const FESpacePtr_Type& fespace )
{
    CurrentFEManifold feBd1 ( fespace->refFE().boundaryFE(), getGeometricMap ( *fespace->mesh() ).boundaryMap() );
...
void
Interpolation::
expandGammaToOmega_Known(const vectorPtr_Type& vectorOnGamma, vectorPtr_Type& vectorOnOmega)
{
...
void
Interpolation::
restrictOmegaToGamma_Known(const vectorPtr_Type& vectorOnOmega, vectorPtr_Type& vectorOnGamma)
{
...
void
Interpolation::
solution (vectorPtr_Type& Solution)
{
    Solution->zero();
...
void
Interpolation::
updateRhs(const vectorPtr_Type& newRhs)
{
    M_RhsF1->zero();
    M_RhsF2->zero();
    M_RhsF3->zero();
